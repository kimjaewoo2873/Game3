...
public class Round3 extends JPanel{
	...
	private ImageIcon enemy3Image = new ImageIcon(img2);
	...
	public Round3(JFrame frame, DialogPane dialog) {
		this.frame = frame;
		this.dialog = dialog;
		setLayout(null);
		dialog.init();
		...
		monsterTimer = new Timer(2000, new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(monCheck == false) {
					...
					monsterThread.start();
					attackThread = new MonsterAttackThread(Round3.this);
					attackThread.start();
					monCheck = true;
				}
				monster = new JLabel();
				i = (int)(Math.random()*2);
				if(i == 0) { // enemy3
					mx = (int)(Math.random()*frame.getWidth() - enemy3Image.getIconWidth() - 30);
	...
	public boolean deleteFixedCheck() { // 고정 몬스터 전용
		...
		for(int i=0;i<monsterVector.size();i++) {
			...
			for(int j=0;j<bulletVector.size();j++) {
				...
				Rectangle monsterBounds = new Rectangle(mon.getX(), mon.getY(), enemy3Image.getIconWidth(), enemy3Image.getIconHeight());
	...
	class MonsterAttackThread extends Thread{
		private Round3 panel;
		private JLabel meteor;
		private JLabel mon;
		private boolean running = true;
		public MonsterAttackThread(Round3 panel) {
			this.panel = panel;
		}
		@Override
		public void run() {
		...
		public void attack() {
			if(monsterVector.size() != 0) {
				for(int i=0;i<monsterVector.size();i++) {
					...
					if(rand == 0) {
						mon = monsterVector.get(i);
						if(mon.getIcon() == enemy3Image) {
							meteor = new JLabel();
							meteor.setIcon(meteorImage);
							meteor.setSize(30, 40);
							meteor.setLocation(mon.getX() + enemy3Image.getIconWidth()/2, mon.getY() + enemy3Image.getIconHeight());
			...
			while(true) {
				try {
					...
					if(pass == true) { // 다이얼로그 패널 바꿔야함
						...
						if(reloadTimer != null)
							reloadTimer.stop();
						Round4 round4 = new Round4(frame,dialog);
						R4DialogPanel r4dialogPanel = new R4DialogPanel(dialog);
						dialog.setContentPane(r4dialogPanel);
						dialog.revalidate();
						dialog.repaint();
						frame.setContentPane(round4);
						frame.revalidate();
						frame.repaint();
						round4.setFocusable(true);
						round4.requestFocus();
						interrupt();
					} 
				} catch (InterruptedException e) {
